...
VENDORED COPY: source of truth is
GreenChanger_data/greenchanger_data/intervention_model.py. This file is
duplicated here because the backend Docker build context only includes
backend/, not GreenChanger_data/. If the data team changes the model
logic or config/intervention_model_parameters.json, re-copy both files.
"""
...
def calculate_intervention_impact(
    ...
    *,
    ...
    status = (
        "indicative_range" if temperature is not None else "evidence_insufficient_for_temperature"
    )
    return {
