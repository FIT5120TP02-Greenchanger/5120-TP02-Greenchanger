    ...
    SELECT species.*,
           NOT (
               ...
               OR NOT (
                   ...
                   AND UPPER(COALESCE(identity_enrichment.taxon_rank, '')) = 'SPECIES'
                   AND identity_enrichment.matched_scientific_name ~
                       '^[A-Z][A-Za-z.-]+([[:space:]]+[x×])?[[:space:]]+[a-z][A-Za-z.-]+'
                   AND NULLIF(BTRIM(identity_enrichment.matched_scientific_name), '') IS NOT NULL
               )
           ) AS has_resolved_tree_identity
    FROM species
    LEFT JOIN tree_species_image_enrichment AS identity_enrichment
      ON LOWER(BTRIM(identity_enrichment.scientific_name)) = species.species_key
), exact_cost AS (
...
SELECT species.scientific_name,
       ...
       CASE
           ...
           WHEN NOT species.has_resolved_tree_identity
               THEN 'The source label is not a resolved botanical identity. It is retained for audit but must be mapped to an exact named taxon before attaching a species quote.'
...
COMMENT ON VIEW complete_tree_species_catalog IS
    'All database and priced catalogue labels. Species quotes are exact or auditable taxon aliases; unresolved common, placeholder and malformed labels remain separate and no generic price is substituted.';
